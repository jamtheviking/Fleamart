...
public class DatabaseHelper extends SQLiteOpenHelper {
   ...
   public Cursor viewAllItems(int userID){
    ...
    //Jo imported List Class/Array
    public List<Item> getUserListedItems(int userId) {
        List<Item> items = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor c = db.query(TABLE_ITEMS,
                new String[]{COLUMN_ITEM_NAME, COLUMN_ITEM_DESCRIPTION, COLUMN_ITEM_PRICE, COLUMN_ITEM_DISCOUNT, COLUMN_ITEM_LOCATION, COLUMN_ITEM_CATEGORY, COLUMN_ITEM_IMAGE},
                COLUMN_USER_ID + "=?",
                new String[]{String.valueOf(userId)}, null, null, null);
        while (c.moveToNext()) {
            Item item = new Item();
            item.setImageData(c.getBlob(c.getColumnIndexOrThrow(COLUMN_ITEM_IMAGE)));
            item.setItemName(c.getString(c.getColumnIndexOrThrow(COLUMN_ITEM_NAME)));
            item.setItemDescription(c.getString(c.getColumnIndexOrThrow(COLUMN_ITEM_DESCRIPTION)));
            item.setItemPrice(c.getDouble(c.getColumnIndexOrThrow(COLUMN_ITEM_PRICE)));
            item.setDiscount(c.getDouble(c.getColumnIndexOrThrow(COLUMN_ITEM_DISCOUNT)));
            item.setLocation(c.getString(c.getColumnIndexOrThrow(COLUMN_ITEM_LOCATION)));
            item.setCategory(c.getString(c.getColumnIndexOrThrow(COLUMN_ITEM_CATEGORY)));
            items.add(item);
        }
        c.close();
        return items;
    ...
    public boolean updateItem(int userId, String itemName, double itemPrice, double itemDiscount, String itemDesc, String itemLocation, String itemCategory) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues items = new ContentValues();
        // Updating values
        items.put(COLUMN_ITEM_NAME, itemName);
        items.put(COLUMN_ITEM_PRICE, itemPrice);
        items.put(COLUMN_ITEM_DISCOUNT, itemDiscount);
        items.put(COLUMN_ITEM_DESCRIPTION, itemDesc);
        items.put(COLUMN_ITEM_LOCATION, itemLocation);
        items.put(COLUMN_ITEM_CATEGORY, itemCategory);

        // updating row
        int rows = db.update(TABLE_ITEMS, items, COLUMN_USER_ID + " = ?", new String[] { String.valueOf(userId)});
        db.close();
        return rows > 0;
    }
