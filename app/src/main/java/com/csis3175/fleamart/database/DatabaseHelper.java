...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // created a ListItem type viewAllItems just to experiment with code for Viewhistory
    public List<Item> viewAllItemsList(int userID) {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                @SuppressLint("Range") double price = c.getDouble(c.getColumnIndex("price"));
                @SuppressLint("Range") byte[] image = c.getBlob(c.getColumnIndex("image"));
                @SuppressLint("Range") boolean isShareable = c.getInt(c.getColumnIndex("isShareable")) > 0;
                @SuppressLint("Range") String date = c.getString(c.getColumnIndex("date"));

                items.add(new Item(name, description, price, image, isShareable, date));
            } while (c.moveToNext());
        }
        c.close();
        return items;
